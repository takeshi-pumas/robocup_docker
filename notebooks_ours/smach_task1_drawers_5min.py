...
class Initial(smach.State):
    ...
    def execute(self,userdata):
        ...
        rospy.loginfo('STATE : Initializing')
...
class Pre_deliver(smach.State):
    ...
    def execute(self,userdata):
        ...
        if now > end_time:
            ...
            return  0
        publish_scene()
        print ("Dekiver to ",userdata.delivery_destination)
...
class OpenDrawer(smach.State):
    def __init__(self, lis, tf_stat, whole_body, arm, gripper, head, move_base_goal):
        smach.State.__init__(self,outcomes=['succ','failed'],input_keys=['timer_in'],output_keys=['timer_out'])
...
if __name__== '__main__':
    ...
    with sm:
        ...
        smach.StateMachine.add("OPENDRAWER",       OpenDrawer(listener, tf_static_broadcaster, whole_body, arm, gripper, head, move_base_goal),      transitions = {'succ':'SCAN_FLOOR', 'failed':'SCAN_FLOOR'},remapping={'timer_in':'sm_timer','timer_out':'sm_timer'})
        #smach.StateMachine.add("INITIAL",       Initial(),      transitions = {'failed':'INITIAL',      'succ':'PRE_DRAWER',    'tries':'INITIAL'}) 
        smach.StateMachine.add("SCAN_FLOOR",    Scan_floor(),   transitions = {'failed':'SCAN_TABLE2',   'succ':'PRE_FLOOR',     'tries':'SCAN_TABLE','change':'SCAN_TABLE2'}) 
